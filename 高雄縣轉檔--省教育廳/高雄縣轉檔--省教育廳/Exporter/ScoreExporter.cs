...
namespace 高雄縣轉檔__省教育廳.Exporter
{
    class ScoreExporter
    {
        ...
        private void FillSubjectSemScores(Workbook wb)
        {
            ...
            int rowIndex = 1;
            //TODO : 以下要重寫
            foreach (string stud_no in DAO.SubjectSemScores.GetAllStudentNos())
            {
                VO.StudentInfo stud = DAO.Students.GetStudentByStudNo(stud_no);
                if (stud == null) continue; //jump to next loop !
                Dictionary<string, Dictionary<string, List<VO.SubjectSemScoreInfo>>> subjScores =  DAO.SubjectSemScores.GetSubjectSemScoresByStudentNo(stud_no);
                foreach (string schoolYear in subjScores.Keys)
                {
                    foreach (string semester in subjScores[schoolYear].Keys)
                    {
                        foreach (VO.SubjectSemScoreInfo subjScore in subjScores[schoolYear][semester])
                        {
                            if (rowIndex > 65535)
                            {
                                //1.建立新的worksheet
                                wsCount += 1;
                                ws = this.createSubjSemScoreWorksheet(wsCount, wsName, wb);
                                //2. 重設 rowIndex 
                                rowIndex -= 65535;
                            }
                            ...
                            ws.Cells[rowIndex, 8].PutValue(subjScore.Hour);
                            ws.Cells[rowIndex, 9].PutValue(subjScore.Hour);
        ...
        private void FillDomainSemScores(Workbook wb)
        {
            ...
            int rowIndex = 1;
            //TODO : 以下要重寫
            foreach (string stud_no in DAO.DomainSemScores.GetAllStudentNos())
            {
                VO.StudentInfo stud = DAO.Students.GetStudentByStudNo(stud_no);
                if (stud == null) continue; //jump to next loop !
                Dictionary<string, Dictionary<string, List<VO.DomainSemScoreInfo>>> domainScores = DAO.DomainSemScores.GetDomainSemScoresByStudentNo(stud_no);
                foreach (string schoolYear in domainScores.Keys)
                {
                    foreach (string semester in domainScores[schoolYear].Keys)
                    {
                        foreach (VO.DomainSemScoreInfo domainScore in domainScores[schoolYear][semester])
                        {
        ...
        private void FillOneDomainScore(VO.DomainSemScoreInfo domainScore, Worksheet ws, int rowIndex, VO.StudentInfo stud)
        {
            ...
            ws.Cells[rowIndex, 4].PutValue(domainScore.GetDomainName());
